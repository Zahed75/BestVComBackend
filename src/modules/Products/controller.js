...
const getProductDetailsController = asyncHandler(async (req,res)=>{
    const { productId } = req.params;
        ...
        const data = await productService.getProductWithOutletQuantities(productId);
...
const getProductOutlets = async (req, res) => {
    const { productId } = req.params;
    ...
    const result = await productService.getProductOutlets(productId);
